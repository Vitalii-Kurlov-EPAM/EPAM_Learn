...
public class CartItemRepository : BaseRepository, ICartItemRepository
{
    ...
    public IEnumerable<CartItemEntity> GetAllCartItems(int cartId, bool includeDeps = false)
    {
        var itemsDbSet = includeDeps
            ? DbSet.Include(x => x.Image)
            : DbSet;
    ...
    public CartItemEntity GetCartItem(int cartId, int cartItemId, bool includeDeps = false)
    {
        var itemsDbSet = includeDeps
            ? DbSet.Include(x => x.Image)
            : DbSet;
