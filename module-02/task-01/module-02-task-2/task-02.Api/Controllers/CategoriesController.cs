...
public class CategoriesController : ControllerBase
{
    ...
    [HttpPost]
    [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(CategoryResponse.CreatedDto))]
    ...
    public async Task<IActionResult> Create(CategoryRequest.CreateModel model)
    {
        ...
        var response = category.ToCategoryResponseCreatedDto(
            new ResourceLink("self", Url.Action("GetById", new { categoryId = category.CategoryId }), "GET"),
            new ResourceLink("delete", Url.Action("DeleteById", new { categoryId = category.CategoryId }), "DELETE"),
            new ResourceLink("put", Url.Action("Update", new { categoryId = category.CategoryId }), "PUT")
        );

        return CreatedAtAction("GetById", new { categoryId = category.CategoryId }, response);
    }
